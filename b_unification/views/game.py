...
from b_unification.serializers import game as game_s
...
class application_meeting_update_view(viewsets.ModelViewSet):
    queryset = game.application_meeting.objects.all()
    permission_classes = [permissions.IsNoBan]
...
class application_meeting_search_view(permissions.ListViewSet):
    queryset = game.application_meeting.objects.order_by('-created_at')
    permission_classes = []
    filter_backends = (DjangoFilterBackend,)

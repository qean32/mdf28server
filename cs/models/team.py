...
class team_CS(models.Model):
    team_name = models.CharField('название команды', max_length=30, default='clown`s', unique=True)
    logo = models.ImageField('лого команды', blank=True, null=True, upload_to='team_CS/logo/')
    background = models.ImageField('фон', blank=True, null=True, upload_to='team_CS/background/')
    ...
    cups = models.ManyToManyField('cup_CS',verbose_name='кубки команды', blank=True)
...
class player_CS(models.Model):
    ...
    team = models.ForeignKey(team_CS, models.SET_NULL, null=True,blank=True)
    generation = models.ForeignKey('generation_CS', models.SET_NULL, null=True, blank=True)
    position = models.ManyToManyField('position_CS', blank=True)
    cups = models.ManyToManyField('solo_cup_CS',verbose_name='кубки команды', blank=True)
    ...
    rank = models.ForeignKey('rank_CS', on_delete=models.PROTECT, null=True, blank=True, default=1)
    pts = models.IntegerField('птс', default=0)
    nickname = models.CharField('никнейм_CS', max_length=55, blank=True, null=True)
...
class offers_CS(models.Model):
    is_view = models.BooleanField('согласие пользователя', default=False)
    user = models.ForeignKey(User, models.CASCADE)
    team = models.ForeignKey(team_CS, on_delete=models.CASCADE)
    generation = models.ForeignKey('generation_CS', models.SET_NULL, null=True)
    position = models.ManyToManyField('position_CS', blank=True)
...
class rank_CS(models.Model):
    rank_name = models.CharField('ранг', max_length=55)
    image_rank = models.ImageField('картинка ранга', blank=True, null=True, upload_to='rank_CS/')
...
class position_CS(models.Model):
    position_name = models.CharField('позиция', max_length=55)
    image_position = models.ImageField('картинка позиции', blank=True, null=True, upload_to='position_CS/')
...
class generation_CS(models.Model):
...
class cup_CS(models.Model):
    name = models.CharField('название', max_length=30,null=True,blank=True)
    image = models.ImageField('лого кубка', blank=True, null=True, upload_to='cup/cs/')
...
class solo_cup_CS(models.Model):
    name = models.CharField('название', max_length=30,null=True,blank=True)
    image = models.ImageField('лого кубка', blank=True, null=True, upload_to='cup_solo/cs/')
